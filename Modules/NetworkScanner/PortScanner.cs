...
public class PortScanner
{
    ...
    public PortScanner(ConfigManager config)
    {
        ...
        _commonPorts = config.CommonServicePorts;
    }

    public async Task<ScanResult> ScanAsync(string target)
    {
        var result = new ScanResult { ModuleName = "Port Scanner", TargetUrl = target, StartTime = DateTime.UtcNow };
        try
        {
            ...
            for (int port = _startPort; port <= _endPort; port++)
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    try
                    {
                        ...
                        if (isOpen)
                        {
                            var service = _commonPorts.TryGetValue(currentPort, out var svc) ? svc : "Unknown";
            ...
            {
                var severity = port switch { 21 or 23 or 139 or 445 or 3389 => "High", 22 or 1433 or 3306 or 5432 or 6379 or 27017 => "Medium", _ => "Low" };
                result.Vulnerabilities.Add(new Vulnerability { Type = "Open Port", Severity = severity, Url = host + ":" + port, Parameter = "Port " + port, Payload = service, Description = "Port " + port + "/tcp is open (" + service + ").", Remediation = "Close unnecessary ports in firewall. Restrict access to sensitive services.", Module = "PortScanner", Confidence = 100 });
            }

            Logger.Info($"Port scan complete: {openPorts.Count} open ports found.");
            result.Completed = true;
        }
        catch (Exception ex) { result.ErrorMessage = ex.Message; Logger.Error("Port scanner failed: " + ex.Message); }
    ...
    {
        try { using var tcpClient = new TcpClient(); using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(2)); await tcpClient.ConnectAsync(host, port, cts.Token); return true; } catch { return false; }
    }
    ...
    private static string ExtractHost(string url)
    {
        try
        {
            if (url.Contains("://")) { var uri = new Uri(url); return uri.Host; }
            var colonIndex = url.LastIndexOf(':');
            if (colonIndex > 0) { var possiblePort = url[(colonIndex + 1)..]; if (int.TryParse(possiblePort, out _)) return url[..colonIndex]; }
            return url;
        }
        catch { return url; }
    }
